...
#ifdef USE_SL3
    // kcg: we need to add even oneway calls,
    // since SL3 transport security is not working
    // w/o this for them -- i.e we're using request
    // hints for getting appropriate security
    // contexts
    add_invoke (rec);
#else // USE_SL3
    if (response_exp) {
...
void
CORBA::ORB::register_option_for_removal(const char* opt, bool arg)
{
    removal_option_map_[opt] = arg;
}
...
//    server_intcpt -> activate (0);
	...
	MICOMT::AutoDebugLock lock;
	MICO::Logger::Stream(MICO::Logger::Security)
	    << "remove SL3 args from ORB_init" << endl;
    }
    MICOGetOpt::OptMap sl3_opts;
    // here we need to remove SL3 parameters, since SL3 interceptor initializers
    // don't have any chance to remove SL3's parameters from supplied
    // argv list.
    // we will also find if SL3 is enabled by -ORBSL3 param
    sl3_opts["-ORBSL3"] = "";
    MICOGetOpt sl3_opt_parser (sl3_opts);
    if (!sl3_opt_parser.parse (rcfile.c_str(), TRUE))
	mico_throw (CORBA::INITIALIZE());
    if (!sl3_opt_parser.parse (argc, argv, TRUE))
	mico_throw (CORBA::INITIALIZE());    

    const MICOGetOpt::OptVec& ovec = sl3_opt_parser.opts ();

    for (MICOGetOpt::OptVec::const_iterator i = ovec.begin(); i != ovec.end(); ++i) {
	string arg = (*i).first;
	string val = (*i).second;
	if (arg == "-ORBSL3") {
	    use_sl3 = TRUE;
	...
	MICOGetOpt::OptMap opts;
	map<string, bool> options = orb_instance->options_for_removal();
	for (map<string, bool>::iterator i = options.begin();
	     i != options.end();
	     i++) {
	    if ((*i).second) {
		opts[(*i).first] = "arg-expected";
	    }
	    else {
		opts[(*i).first] = "";
	    }
	...
	if (!opt_parser.parse (argc, argv, TRUE))
	    mico_throw (CORBA::INITIALIZE());
	// option processing finished. All registered options
	// should be deleted from argc/argv at this point
	delete info;
    }
