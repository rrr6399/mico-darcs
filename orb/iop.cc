...
// kcg: needed only for debuging
//      don't forget to remove it in final stage
...
{
    //cerr << "~GIOPConn() to " << _transp->peer()->stringify() << endl;
    if (_refcnt != 0 ) {
	if (MICO::Logger::IsLogged (MICO::Logger::GIOP)) {
	    MICOMT::AutoDebugLock __lock;
	    MICO::Logger::Stream (MICO::Logger::GIOP)
...
#endif // HAVE_THREADS
#ifdef USE_SL3
    if (!CORBA::is_nil(init_ctx_)) {
	InitiatingContext_var ctx = InitiatingContext::_narrow(init_ctx_);
	assert(!CORBA::is_nil(ctx));
	ctx->shutdown();
	MICOSL3_TransportSecurity::TransportCredentials_impl* tscimpl
	    = dynamic_cast<MICOSL3_TransportSecurity::TransportCredentials_impl*>
	    (ctx.in());
	assert(tscimpl != NULL);
	tscimpl->creds_state(SL3CM::CS_Invalid);
	MICOSL3_TransportSecurity::InitiatingContext_impl* icimpl
	    = dynamic_cast<MICOSL3_TransportSecurity::InitiatingContext_impl*>
	    (init_ctx_.in());
	assert(icimpl != NULL);
	icimpl->notify_close_context();
    }
    if (!CORBA::is_nil(accept_ctx_)) {
	MICOSL3_TransportSecurity::TransportCredentials_impl* tscimpl
	    = dynamic_cast<MICOSL3_TransportSecurity::TransportCredentials_impl*>
	    (accept_ctx_.in());
	assert(tscimpl != NULL);
...
{
    CORBA::Object_ptr obj = static_cast<CORBA::Object*>
        (value);
    if (!CORBA::is_nil(obj)) {
        CORBA::release(obj);
    }
...
void
IIOPServer_cleanup_idrec_map(void* value)
{
    IIOPServer::ThreadIdRecMapMember* obj = static_cast<IIOPServer::ThreadIdRecMapMember*>
        (value);
    if (obj != NULL) {
        // this just marks map for deletion and actual deletion will
        // be done by different thread in ::kill_conn method
        obj->scheduled_for_removal_ = TRUE;
    }
}
...
{
#ifdef HAVE_THREADS
    MICOMT::Thread::create_key(idrec_map_key_, IIOPServer_cleanup_idrec_map);
#endif // HAVE_THREADS
    // require at most one IIOPServer running
    assert(iiopserver_instance_ == NULL);
    // initialize static instance
    iiopserver_instance_ = this;
...
MICO::IIOPServer::~IIOPServer ()
{
	...
	for (ThreadIdRecMap::iterator i1 = _orbids.begin();
	     i1 != _orbids.end(); ++i1) {
            ...
            for (MapIdConn::iterator j = (*i1).second->map_.begin();
...
CORBA::Boolean
MICO::IIOPServer::listen (CORBA::Address *addr, CORBA::Address *fwproxyaddr, const CORBA::Address*& bound_addr)
{
    CORBA::IORProfile *prof;
...
void
MICO::IIOPServer::shutdown(CORBA::Address* addr)
{
...
MICO::IIOPServer::pull_invoke_reqid (MsgId msgid, GIOPConn *conn)
{
    ...
    for (ThreadIdRecMap::iterator i = _orbids.begin(); i != _orbids.end(); ++i) {
        ...
        for (MapIdConn::iterator j = (*i).second->map_.begin();
             j != (*i).second->map_.end();
             j++) {
            rec = (*j).second;
            if (rec->reqid() == msgid && rec->conn() == conn && rec->active() ) {
...
#ifdef HAVE_THREADS
    MICOMT::Locked<ThreadIdRecMapMember>* map = static_cast<MICOMT::Locked<ThreadIdRecMapMember>*>
        (MICOMT::Thread::get_specific(idrec_map_key_));
    if (map == NULL) {
        map = new MICOMT::Locked<ThreadIdRecMapMember>();
...
MICO::IIOPServer::del_invoke_orbid (IIOPServerInvokeRec *rec)
{
    ...
    if (map != NULL) {
        MICOMT::AutoLock l(*map);
        MapIdConn::iterator i = map->map_.find(rec->orbmsgid());
        if (i != map->map_.end()) {
            found = true;
            delete (*i).second;
            map->map_.erase(i);
...
MICO::IIOPServer::del_invoke_reqid (MsgId msgid, GIOPConn *conn)
{
    ...
    for (ThreadIdRecMap::iterator i = _orbids.begin(); i != _orbids.end(); i++) {
        MICOMT::AutoLock l2(*(*i).second);
        for (MapIdConn::iterator j = (*i).second->map_.begin(); j != (*i).second->map_.end(); j++) {
            rec = (*j).second;
            if (rec->reqid() == msgid && rec->conn() == conn) {
                assert( !rec->active() );
                delete rec;
...
MICO::IIOPServer::kill_conn (GIOPConn *conn, CORBA::Boolean redo)
{
	...
	for (MapIdConn::iterator i = _orbids.begin(); 
        ...
        for (ThreadIdRecMap::iterator i = _orbids.begin();
            ...
            if (MICO::MTManager::thread_per_connection()
                && ((*i).second->map_.empty())
                && ((*i).second->scheduled_for_removal_)) {
                    // purge empty map
                    delete (*i).second;
                    _orbids.erase(i);
                    again = TRUE;
...
MICO::IIOPServer::shutdown (CORBA::Boolean wait_for_completion)
{
...

    {
        ...
        for (ThreadIdRecMap::iterator i = _orbids.begin();
             i != _orbids.end();
             i++) {
            ThreadIdRecMapMember* map = (*i).second;
            {
                MICOMT::AutoLock l2(*(*i).second);
                for (MapIdConn::iterator j = (*i).second->map_.begin();
                     j != (*i).second->map_.end();
            ...
            map->map_.erase(map->map_.begin(), map->map_.end());
            delete map;
        }
        _orbids.erase (_orbids.begin(), _orbids.end());
#endif // HAVE_THREADS
