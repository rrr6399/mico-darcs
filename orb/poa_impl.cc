...
#ifdef USE_SL3
    else if (pol->policy_type() == TransportSecurity::ObjectCredentialsPolicyType) {
    ...
    : name (_name), parent(_parent), orb(porb), ObjectActivationLock(FALSE, MICOMT::Mutex::Recursive)
{
  ...
  if (!CORBA::is_nil(ocp)) {
      CORBA::IORProfile* tprof = NULL;
      TransportSecurity::OwnCredentialsList_var creds = ocp->creds_list();
      for (CORBA::ULong i = 0; (tprof = ior_template_->get_profile(i)) != NULL; i++) {
          bool match = false;
          if (tprof->id() == CORBA::IORProfile::TAG_INTERNET_IOP) {
              MICO::IIOPProfile* iiop_prof = dynamic_cast<MICO::IIOPProfile*>(tprof);
              assert(iiop_prof != NULL);
              for (CORBA::ULong j = 0; j < creds->length(); j++) {
                  TransportSecurity::CredentialsAcceptor_var acceptor
                      = creds[j]->the_acceptor();
          ...
          else if (tprof->id() == CORBA::IORProfile::TAG_SSL_INTERNET_IOP) {
              MICOSSL::SSLProfile* ssl_prof = dynamic_cast<MICOSSL::SSLProfile*>(tprof);
              assert(ssl_prof != NULL);
              for (CORBA::ULong j = 0; j < creds->length(); j++) {
                  TransportSecurity::CredentialsAcceptor_var acceptor
                      = creds[j]->the_acceptor();
          ...
          else if (tprof->id() == CORBA::IORProfile::TAG_MULTIPLE_COMPONENTS) {
              // we do not want to remove multiple components profile
              match = true;
          }
          if (!match) {
              // the selected profile does not match OCP credentials
              // so we need to remove it
              ior_template_->del_profile(tprof);
              // we also need to decrease i!
              i--;
          }
      }
  }
